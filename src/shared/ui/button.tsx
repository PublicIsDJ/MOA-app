...
// MARK: 버튼 컴포넌트
export function Button({status, children, className='', type='button', ...props}: Props) {
    ...
    return (
        <button className={`${utilsStyle} ${className} ${status === 'default' ? defualtStyle : inactiveStyle}`}>
            {children}
        </button>
    );
